...
def pivot_months_pandas(data):
    ...
    # Adding column 'month' that contains results of applying date_to_month
    data['month'] = date_to_month(data['date'])

    # Aggregate over 'name' and 'month' columns
    grouped_data = data.groupby(['name','month'])
    aggr_sum =grouped_data.aggregate('sum').reset_index()

    # Using pivot method to create a row for each station and column for each month
    monthly = aggr_sum.pivot(index='name', columns='month')['precipitation']

    # Repeat with 'count' aggregation
    aggr_count = grouped_data.aggregate('count').reset_index()
    counts = aggr_count.pivot(index='name', columns='month')['precipitation']
