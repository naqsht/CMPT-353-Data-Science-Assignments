...
def main(infile):
    ...
    # Create and fit RGB model using GaussianNB()
    model_rgb = GaussianNB()
    model_rgb.fit(X_train, y_train)
    model_rgb_score = model_rgb.score(X_test, y_test)

    # TODO: print model_rgb's accuracy score
    print("model_rgb score = " + str(model_rgb_score))

    # TODO: build model_lab to predict y from X by converting to LAB colour first.
    # Create and fit LAB model usinf make_pipeline()
    model_lab = make_pipeline(FunctionTransformer(convert_RGBtoLAB), GaussianNB())
    model_lab.fit(X_train, y_train)
    model_lab_score = model_lab.score(X_test, y_test)

    # TODO: print model_lab's accuracy score
    print("model_lab score = " + str(model_lab_score))
